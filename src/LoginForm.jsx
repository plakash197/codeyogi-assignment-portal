...
function LoginForm(props) {
    ...
    if (response?.credential) {
      ...
      if (foundUser) {
        if (foundUser.role.toLowerCase() !== props.name.toLowerCase()) {
          alert(`This email is registered as a ${foundUser.role}. You can't login here!`);
          return;
        }
      } else {
        const newUser = {
  ...
  useEffect(() => {
    if (window.google) {
      window.google.accounts.id.initialize({
        client_id: '229434297162-n2s7bst2pfpn6gekct5e59agegourtb.apps.googleusercontent.com',
  ...
  function handleLogin() {
    ...
    if (checkGoogleUser) {
      alert('This email is registered via Google login. Please login via Google.');
      return;
    }
  ...
  function handleForgetPassword() {
    const uName = prompt('Enter your name which was given when you created the account')?.toLowerCase();
    const uEmail = prompt('Enter your email id which you use to login')?.toLowerCase();

    if (!uName || !uEmail) return;

    const userList = JSON.parse(localStorage.getItem('manualUser') || '[]');
    const userIndex = userList.findIndex(
      (u) => u.name?.toLowerCase() === uName && u.email?.toLowerCase() === uEmail
    );

    if (userIndex !== -1) {
      const newPass = prompt('Enter your new password: Minimum Length: 8 and maximum Length: 16');
      if (!newPass) return;

      if (newPass.length < 8) {
        alert('Sorry we cant set this password because password length is less then 8, Try Again');
        return;
      } else if (newPass.length > 16) {
        alert('Sorry we cant set this password because password length greater than 16, Try Again');
        return;
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 flex flex-col justify-between p-4 sm:p-8 lg:p-12">
        ...
        <div className="w-full max-w-md lg:max-w-lg mx-auto bg-white rounded-3xl shadow-2xl p-6 sm:p-10 my-8 border border-gray-100 transition-all">
          ...
          <div className="space-y-5">
            ...
            <div id="googleLoginBtn" className="flex justify-center w-full overflow-hidden rounded-xl"></div>
